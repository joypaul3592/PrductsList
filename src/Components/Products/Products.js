...
const Products = ({ handelFun }) => {
    ...
    const handelCount = ({ value, id }) => {
        if (value) {
            const ProductCounts = [{ value: value, id: id }]
            const ProductCount = proCount.filter(p => p.id == id);
            console.log(ProductCount)
            if (ProductCount.length) {
                console.log('if');
                const objIndex = proCount.findIndex((obj => obj.id == id));
                proCount[objIndex].value = value;
            } else {
                console.log('else')
                proCount.push(...ProductCounts)
            }
        }
    }
                ...
                show ? <>
                    ...
                    <div className=' flex items-center justify-between  py-3 '>
                        ...
                        <div className=' flex items-center'>
                            ...
                            <div>
                                <button onClick={() => { handelCart(data, proCount) }} className=' py-[6px] rounded-sm px-3 bg-sky-400 text-white text-sm font-medium'>Add To Cart</button>
                                <button onClick={() => handelFun(1)}>Add</button>
                            </div>
                        </div>
                    </div>
                    <div>
                            ...
                            filterValue.map(product =>
                                ...
                                <div key={product?.id} className=' flex items-center justify-around bg-white shadow-md my-10'>
                                    ...
                                    <from className=' flex items-center '>
                                        ...
                                        <label onClick={() => handelValue({ id: product?.id, })} className=' cursor-pointer py-1 flex mx-1' htmlFor={product?.name}> <BsFillCartDashFill />  <input className='bg-white ml-3' type="checkbox" id={product?.name} name={product?.name} value={product?.name} /> </label>
